...
INPUT     ?= data/processed/edges_wormwiring_ISM.csv
PORT      ?= 8000
...
V4 := data/processed/V4/graph_V4_directed_weighted.gpickle
...
help:
	...
	@echo "  make preprocess INPUT=data/processed/edges_wormwiring_ISM.csv"

# ---- Preprocess (produces all 3 graphs + summary) ----
.PHONY: preprocess
preprocess: $(V1) $(V2) $(V3) $(SUMMARY) $(V4)

$(V1) $(V2) $(V3) $(SUMMARY):
	$(PY) src/pipeline_prepare_graphs.py --config $(CFG) --input $(INPUT)

# V4 requires V2 to be generated first, then adds coordinates
$(V4): $(V2)
	$(PY) src/generate_v5.py --config $(CFG) --base-variant V2 --seed 42
...
$(E2_FIG): $(V4)
	$(PY) src/e2_dijkstra/run_sp.py --config $(CFG) --variant V4
