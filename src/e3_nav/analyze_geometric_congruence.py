...
# -*- coding: utf-8 -*-
"""
Analyze E3 experiment's Geometrical Congruence (GC) and Greedy Routing Efficiency (GRE)
...
def main():
    ...
    ap.add_argument("--sample_size", type=int, default=500, help="Number of node pairs to sample (for GC computation and scatter plots)")
    ...
    # If results have few data, sample more node pairs from graph and run navigation experiments
    if len(node_pairs_from_results) < args.sample_size:
        ...
        if sample_size > len(node_pairs_from_results):
            ...
            # Run navigation experiments on additionally sampled node pairs
    ...
    if 'high_gc_success_rate' in analysis and not np.isnan(analysis['high_gc_success_rate']):
        print(f"\nGC Group Analysis (Median Split):")
    ...
    # Figure 1: GC distribution - use more bins
    if len(gc_values) > 0:
        fig, ax = plt.subplots(figsize=(10, 6))
        n_bins_hist = min(50, max(20, len(gc_values) // 5))  # More bins
    ...
    if 'merged' in analysis and len(analysis['merged']) > 0:
        ...
        # If there are additional GC data (without navigation results), also include in scatter plot
        if 'gc_df' in analysis and len(analysis['gc_df']) > len(merged):
            ...
            # Mark which ones have navigation results
            all_gc_data['has_nav_result'] = all_gc_data['success'].notna()
        else:
            all_gc_data = merged
            all_gc_data['has_nav_result'] = True
        
        # Binning analysis - use more bins
        fig, axes = plt.subplots(1, 2, figsize=(14, 5))
        
        # Left plot: GC vs Success Rate (binned) - use more bins
        ax1 = axes[0]
        # Use all GC data (including those without navigation results)
        gc_data_for_binning = all_gc_data[all_gc_data['has_nav_result']] if 'has_nav_result' in all_gc_data.columns else merged
        
        # Dynamically adjust number of bins based on data volume, but at least 10 bins
        n_bins = min(30, max(10, len(gc_data_for_binning) // 5))  # More aggressive bin count
        ...
        # Only show bins with sufficient samples (at least 2 samples, lower threshold to show more points)
        min_samples = max(1, len(gc_data_for_binning) // 50)  # Dynamically adjust minimum sample size
        ...
        # Add sample size annotations (only annotate some points to avoid crowding)
        if len(bin_centers) <= 20:
            for i, (x, y, count) in enumerate(zip(bin_centers, bin_means, bin_counts)):
                if i % max(1, len(bin_centers) // 8) == 0:  # Annotate more points
        ...
        # Right plot: GC vs Stretch (successful paths) - show all data points
        ax2 = axes[1]
        ...
        if len(successful) > 0:
            ...
            if len(gc_success) > 0:
                # Adjust scatter size and transparency based on number of data points
            ...
            # If no successful paths, show GC values for all paths
        ...
        # Create a separate scatter plot showing all GC values (now all points have navigation results)
        if 'gc_df' in analysis and len(analysis['gc_df']) > 0:
            fig2, axes2 = plt.subplots(1, 2, figsize=(16, 6))
            
            # Show all GC values (now all points have navigation results)
            all_gc = all_gc_data['gc'].values
            # Now all points should have navigation results
            has_result = all_gc_data['has_nav_result'].values if 'has_nav_result' in all_gc_data.columns else np.ones(len(all_gc), dtype=bool)
            
            # Left plot: GC distribution scatter plot (all data points)
            ax3 = axes2[0]
            
            # Add small random y offset to avoid point overlap
            np.random.seed(42)
            y_offset = np.random.normal(0, 0.02, len(all_gc))
            
            # Use different colors for points with navigation results
            ...
            # Draw scatter plot (now all points have navigation results, so no need to show gray points)
            ...
            if len(gc_with_result) > 0:
                # If there are navigation results, show success/failure (success in green on top, failure in red below)
                ...
                # Draw failed points first (below)
                ...
                # Then draw successful points (on top)
            ...
            # Right plot: GC vs Success Rate (binned to show trend, more clearly showing higher GC leads to higher success rate)
            ax4 = axes2[1]
            if len(gc_with_result) > 0:
                ...
                # Use quantile binning to ensure roughly equal sample sizes per bin (increase bin count for finer intervals)
                n_bins = 40  # Increased from 20 to 40 for finer intervals
                ...
                min_samples = max(3, len(result_data) // 50)  # Lower minimum sample requirement to accommodate more bins
                valid_bins = bin_stats['count'] >= min_samples
                bin_centers = [bc for bc, valid in zip(bin_centers, valid_bins) if valid]
                bin_means = bin_stats['mean'][valid_bins].values * 100
                bin_counts = bin_stats['count'][valid_bins].values
                ...
                # Draw binned success rate curve (use smaller markers to accommodate more points)
                ...
                # Add raw data points (lower transparency)
                ...
                # Linear regression fit (using binned data)
                if len(bin_centers) > 2:
                    ...
                    # Can also add smooth trend line (optional)
                ...
                # Add statistical information (including R²)
                if len(bin_centers) > 2:
                    X_fit = np.array(bin_centers).reshape(-1, 1)
                    y_fit = bin_means
                    lr = LinearRegression()
